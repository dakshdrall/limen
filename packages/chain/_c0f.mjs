...
import { externalSigner, invokeContract, structMap } from './dist/index.js';
...
const rec=await server.simulateTransaction(await build(500000));
const payload=pay([{signer:externalSigner(VERIFIER,O),signature:new Uint8Array(0)},
                   {signer:externalSigner(VERIFIER,A),signature:new Uint8Array(0)}],[0,1]);
...
const sim=await server.simulateTransaction(await build(500000,attached));
