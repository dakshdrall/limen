...
| `DATABASE_URL` | There are no accounts. `/api/auth/session` answers 503, the header renders no sign-in control at all, and every other screen is unaffected — fixtures, ingest, derivation and install all work with no database, which is what *no credentials are required* above means. Neon's pooled connection string; the web app reaches it over `neon-http`, which has no connection to exhaust. |
